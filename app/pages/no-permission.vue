    ...
    <div class="max-w-md w-full mx-4">
      <div class="bg-white rounded-lg shadow-lg p-8 text-center">
        ...
        <div class="mb-6">
          ...
          <!-- Additional Info -->
          <div v-if="additionalInfo" class="bg-orange-50 border border-orange-200 rounded-lg p-4 text-left">
            <h3 class="font-medium text-orange-800 mb-2">{{ t('no_permission.additional_info') }}</h3>
            <p class="text-sm text-orange-700">{{ additionalInfo }}</p>
          </div>

          <!-- Required Permissions -->
          <div v-if="requiredPermissions.length" class="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left mt-4">
            <h3 class="font-medium text-blue-800 mb-2">{{ t('no_permission.required_permissions') }}</h3>
            <ul class="text-sm text-blue-700 space-y-1">
              <li v-for="permission in requiredPermissions" :key="permission" class="flex items-center">
        ...
        <div v-if="userInfo" class="mt-6 pt-4 border-t border-gray-200">
          ...
          <p class="text-xs text-gray-500 mb-1">
            {{ t('no_permission.current_roles') }}: {{ useOidcAuth().user.value?.claims || 'Not available' }}
          </p>
          <p class="text-xs text-gray-500">
            {{ t('no_permission.request_id') }}: {{ requestId }}
          </p>
        </div>
      </div>
    </div>
...
useHead({
  title: 'Access Denied - Bill24 Admin Portal',
  meta: [
...
const permissionMessage = computed(() => {
  ...
  switch (permissionType.value) {
    ...
    case 'action':
      return t('no_permission.action_message', { action: action.value || 'this action', resource: resourceText })
    default:
...
const requestAccess = () => {
  ...
  const subject = encodeURIComponent('Bill24 Admin Portal - Access Request')
