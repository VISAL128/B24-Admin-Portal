...
class FeeConfiguration {
  ...
  calculateFees(currency: string, amount: number): TransactionResult {
    ...
    for (const row of config.transaction_fees) {
      ...
      if (row.unlimited || (amount >= row.start_amount && (row.end_amount === null || amount <= row.end_amount))) {
        applicableRow = row;
        transactionFee = row.fee_type === 'percentage' ? (amount * row.fee_amount) / 100 : row.fee_amount;
        break;
      }
    }
    ...
    if (applicableRow && config.allocate_details.length > 0) {
      for (const supplier of config.allocate_details) {
        ...
        if (supplierFee) {
          ...
          distribution[supplier.name] = (transactionFee * supplierFeePercentage) / 100;
        }
      }
    }
