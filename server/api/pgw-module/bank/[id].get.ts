...
import type { BankDetailsResponse } from '~/models/bank'
import type { PgwModuleResponse } from '~/models/baseModel'

export default defineEventHandler(
  async (event): Promise<PgwModuleResponse<BankDetailsResponse | null>> => {
    try {
      // Call the PGW Module API
      const response = await requestToPgwModuleApi<PgwModuleResponse<BankDetailsResponse>>(
        event,
        PGW_MODULE_API_ENDPOINTS.BANK.DETAILS.replace('{id}', event.context.params?.id || ''),
        'GET'
      )
